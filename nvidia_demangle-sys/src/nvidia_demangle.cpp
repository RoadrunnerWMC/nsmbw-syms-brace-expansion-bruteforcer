...
extern "C" void demangle(char *res, size_t bufsize, const char *mangled);

void demangle(char *res, size_t bufsize, const char *mangled) {
    std::string demangled = demangle(mangled);
    size_t sz = std::min(bufsize - 1, demangled.size());
    memcpy(res, demangled.c_str(), sz);
    res[sz] = 0;
}
